...
def el_consuelo():
    ...
    print(f"🔍 Procesando columna de limpieza: '{columna_limpieza}'")
    print(f"📊 Total de filas disponibles: {len(global_data)}")
    
    # Procesar solo las filas 2 a 440 (índices 1 a 439 en Python)
    filas_a_procesar = min(440, len(global_data))
    filas_inicio = 1  # Empezar desde la fila 2 (índice 1)
    
    print(f"🎯 Procesando filas {filas_inicio + 1} a {filas_a_procesar}")
    ...
    print(f"📈 Conteos finales de limpieza: {dict(limpieza_counts)}")
